...
class HomeController {
  ...
  async index(req, res) {
    ...
    const countRow = await Post.countSearch(q || null);
    ...
    return renderView(res, req, 'home', {
      title: 'Latest stories',
      lead: 'Stories from the canopy desk.',
      posts,
      q,
  ...
  forgotForm(req, res) {
    return renderView(res, req, 'forgot_password', { title: 'Forgot password' });
  }
  ...
  async forgot(req, res) {
    ...
    if (user) {
      ...
      const link = `${process.env.APP_URL || 'http://127.0.0.1:3002'}/reset-password/${user.reset_token}`;
  ...
  resetForm(req, res) {
    return renderView(res, req, 'reset_password', { title: 'Reset password', error: null });
  }

  async reset(req, res) {
    const user = await User.query().where({ reset_token: req.params.token }).first();
    if (!user || !user.reset_expires || new Date(user.reset_expires) < new Date()) {
      return renderView(res, req, 'reset_password', { title: 'Reset password', error: 'Invalid or expired token' });
    }
    const password = String(req.body.password || '');
    if (password.length < 8) {
      return renderView(res, req, 'reset_password', { title: 'Reset password', error: 'Password too short' });
    }
